...
export default function AdminProjectForm() {
  ...
  const { projects, addProject, updateProject, addGalleryImage, removeGalleryImage } = useAdmin();
  ...
  // Upload local image → convert to Data URL with canvas compression
  function handleFileUpload(e, type) {
    ...
    setUploading(true);
    const size = type === 'cover' ? 1000 : 800;
    compressImage(file, size, 0.7)
      .then((compressedBase64) => {
        if (type === 'cover') {
          setField('image', compressedBase64);
        } else {
          if (isEdit) {
            addGalleryImage(id, compressedBase64);
          } else {
            setForm((f) => ({ ...f, gallery: [...f.gallery, compressedBase64] }));
          }
        }
        setUploading(false);
      })
          ...
          <Section title={`Gallery Images (${currentGallery.length})`}>
            ...
            <div className="mt-3">
              <input ref={fileInputRef} type="file" accept="image/*" multiple className="hidden" onChange={(e) => {
                Array.from(e.target.files).forEach((_f, idx) => {
                  // handle each file
                });
